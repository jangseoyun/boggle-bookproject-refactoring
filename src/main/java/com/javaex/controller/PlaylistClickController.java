...
public class PlaylistClickController {
    ...
    public ResponseEntity<List<PlayListDto>> popularPlayList(@RequestParam("nickname") String nicknameParam,
                            Authentication authentication) {

        UserDto loginUser = userService.findByUserEmail(authentication.getName());
        log.info("likePlayList => 로그인사람의 닉네임:{}, 지금 서재 닉네임:{}", loginUser.getNickname(), nicknameParam);

        List<PlayListDto> popularPlayList = playlistClickService.popularPlayList();
        return ResponseEntity
                .status(HttpStatus.OK)
	...
	public ResponseEntity<List<PlayListDto>> playListByUser(@RequestParam("nickname") String nicknameParam) {

        UserDto pageUser = userService.findByNickname(nicknameParam);
        log.info("likePlayList => 지금 페이지 닉네임:{}", nicknameParam);

        List<PlayListDto> playListByUser = playlistClickService.playListByUser(pageUser.getUserNo());
